...
- **Roles** (Knife Terminal menu → "Run Jobs on This Mac"): one Mac is the **executor** (the Mac mini — publishes its tabs, answers the phone, runs every job). Every other Mac is a **client**: it never publishes tabs or runs jobs; its sidebar shows the executor's sessions and an "ask the executor…" box. The iPhone is always a client (the "ask" box at the top of the list — dictate with the keyboard mic).
- **Manifest**: every Mac publishes its recent projects with the **git remote** (the project's identity across machines), a one-line **description** (README first paragraph, else `claude -p --model haiku` writes one, cached in `~/.knife/descriptions.json`) and **last touched** time. The merged view lands in `~/.knife/manifest.json`. Projects without a remote can't be routed to.
...
- **Execution**: each job is a terminal tab on the executor (so it mirrors to the phone/laptop like any session, and a hung job never touches the app). The runner resolves the remote to a local checkout (cloning into `~/.knife/projects/` if absent), fetches, adds a worktree at `origin/<default branch>`, runs `claude -p --dangerously-skip-permissions`, squashes whatever happened into **one commit**, and follows Claude's verdict: `direct` → push to the default branch (the notification says `git revert <sha>`), `pr` → push a `knife/<job>` branch and `gh pr create`. A rejected direct push falls back to a PR. The full diff is printed in the tab, saved to `~/.knife/jobs/<job>.diff`, and the first 2.5 KB rides in the push notification.
...
- No CloudKit schema change: a request is an `Open` record with a `job:` prefix, the manifest is one `Projects` record per machine (`projects-<host>`), jobs are ordinary `Tab` records. The Mac mini is the sole executor, so there is no job claiming; if multi-machine execution is ever added, iCloud is the wrong coordination layer (eventually consistent) — claiming needs an atomic write, e.g. a small HTTP endpoint on the executor.
